...
function formStateProvider(Form) {
    return /** @class */ (function (_super) {
        ...
        FormStateProvider.prototype.invoke = function (name, handler, onResolve, onReject) {
            var onHandled = (this.props.inspector != null) ? this.props.inspector : function () { };
            var result = handler();
            if (Promise.resolve(result) === result) {
                onHandled({ name: name, type: 'handled', async: true });
                result.then(function () {
                    onResolve();
                    onHandled({ name: name, type: 'resolved', async: true });
                }, function (reason) {
                    // This component doesn't catch Error.
                    if (reason instanceof Error)
                        throw reason;
                    onReject(reason);
                    onHandled({ name: name, type: 'rejected', async: true });
                });
            }
            else {
                onHandled({ name: name, type: 'handled', async: false });
                if (result != null) {
                    onReject(result);
                    onHandled({ name: name, type: 'rejected', async: false });
                }
                else {
                    onResolve();
                    onHandled({ name: name, type: 'resolved', async: false });
                }
            }
        };
        FormStateProvider.prototype.submit = function (event) {
            ...
            this.invoke('form', function () { return _this.props.submitHandler(_this.state.values, event); }, function () { return _this.endSubmitting(); }, function (reason) {
        ...
        FormStateProvider.prototype.invokeValidator = function (name, newValue) {
            ...
            if (this.props.validators != null) {
                ...
                if (validator_1 != null) {
                    ...
                    this.invoke(name, function () { return validator_1(validating_1); }, function () { return _this.cancelError(name); }, function (reason) { return _this.setErrors(reason, name); });
                }
            }
        };
